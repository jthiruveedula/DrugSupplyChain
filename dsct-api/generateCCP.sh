...
sleep 5
  ...
  NO_OF_ORDERERS=1
  echo ${NO_OF_ORDERERS}
  CPWD="/home/paperspace/HTF/HLF_1.4.3"
  SourcePATH="/home/paperspace/HTF/HLF_1.4.3/dsct-api"
  # CPWD="/usr/CLIF"
  ...
  orgDetails[0,2]="Manufacturer"
  orgDetails[1,2]="Delear"
  orgDetails[2,2]="Hosp"
  orgDetails[0,1]=1
  orgDetails[1,1]=1
  orgDetails[2,1]=1
  CCC_NAME="dsctcc"
  CHANNELS[0,0]="dsctchannel"
IP=$(hostname -I|awk {'print $1'})
 ...
 MOPath="${CPWD}/"
  cd $MOPath
  ordererpath="${MOPath}crypto-config/ordererOrganizations/dsct.com/orderers"



  for count in `seq 0 $Mxca`  
  do 
  Orderer_Ca[${count}]="$(awk '{printf "%s\\n", $0}' ${ordererpath}/orderer.dsct.com/tls/ca.crt)"
...
do 
 MOPATH="${CPWD}/"
 Org_Connection[${orgDetails[${countcon},0]}cacert]="$(awk '{printf "%s\\n", $0}' ${MOPATH}crypto-config/peerOrganizations/${orgDetails[${countcon},0]}.dsct.com/ca/ca.${orgDetails[${countcon},0]}.dsct.com-cert.pem)"
 Org_Connection[${orgDetails[${countcon},0]}peercert]="$(awk '{printf "%s\\n", $0}' ${MOPATH}crypto-config/peerOrganizations/${orgDetails[${countcon},0]}.dsct.com/peers/peer0.${orgDetails[${countcon},0]}.dsct.com/tls/ca.crt)"
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
            "peer${peercount}.${orgDetails[${peer},0]}.dsct.com": {
            "endorsingPeer": true,
            "chaincodeQuery": true,
            "eventSource": true
                  }
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
            "peer${peercount1}.${orgDetails[${peer},0]}.dsct.com": {
            "endorsingPeer": true,
            "chaincodeQuery": true,
            "eventSource": true
          },
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json          
            "peer${peercountorder}.${orgDetails[${countorder},0]}.dsct.com"
        ],
EOF
          else
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json          
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json 
        "certificateAuthorities": [
          "ca.${orgDetails[${countorder},0]}.dsct.com"
        ]
EOF
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json 
    "orderer.dsct.com": {
      ...
      "grpcOptions": {
        "ssl-target-name-override": "orderer.dsct.com",
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
      ...
      "grpcOptions": {
        "ssl-target-name-override": "peer${peercount}.${orgDetails[${peer},0]}.dsct.com",
        "hostnameOverride": "peer${peercount}.${orgDetails[${peer},0]}.dsct.com"
      },
      "tlsCACerts": {
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
    "ca.${orgDetails[${countco},0]}.dsct.com": {
      "url": "http://${IP}:${ca_port}",
      "caName": "ca.${orgDetails[${countco},0]}.dsct.com",
      "httpOptions": {
        "verify": false
      },
      "tlsCACerts": {
