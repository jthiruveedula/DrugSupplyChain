...
async function main() {
  try {
    ...
    const userExists = await wallet.exists("ManufacturerUser");
    if (userExists) {
      console.log(
    ...
    const adminExists = await wallet.exists("admin-Manufacturer");
    if (!adminExists) {
      const caInfo = ccp.certificateAuthorities["ca.manufacturer.dsct.com"];
      console.log(caInfo);
      ...
      await wallet.import("admin-Manufacturer", identity);
      main();
      return;
    }
    ...
    await wallet.import("ManufacturerUser", userIdentity);
    console.log(
